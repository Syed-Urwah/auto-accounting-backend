...
export class GeneralJournalService {
  ...
  async get(companyId: number, paginationDto: PaginationDto) {
    const { page, limit } = paginationDto;
    const skip = (page - 1) * limit;

    const [generalJournals, total] = await this.generalJournalRepository.findAndCount({
    ...
    return apiResponse(HttpStatus.OK, 'General Journal and entries retrieved successfully', {
